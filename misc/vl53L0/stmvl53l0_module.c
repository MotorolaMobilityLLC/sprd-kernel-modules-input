...
static void stmvl53l0_work_handler(struct work_struct *work)
{
	...
	if (vl53l0_dev->enable_ps_sensor == 1) {
		...
		vl53l0_dbgmsg("before interruptStatus==gpio_function\n");

		if (vl53l0_dev->interruptStatus == vl53l0_dev->gpio_function) {
			vl53l0_dbgmsg("before GetRangingMeasurementData\n");
			Status =
				papi_func_tbl->GetRangingMeasurementData(
						vl53l0_dev,
						&(data->rangeData));
			...
			if (Status == VL53L0_ERROR_NONE) {
				...
				pr_err("after GetRangingMeasurementData OK\n");
			} else {
				pr_err("%s(%d) : Status = %d\n",
						__func__, __LINE__, Status);
				pr_err("after GetRangingMeasurementData NG\n");
			}
...
int stmvl53l0_setup(struct stmvl53l0_data *data)
{
	...
	/*add identify flow for vl53l0 before create device file*/
	VL53L0_DEV vl53l0_dev = data;
	...
	rc = VL53L0_RdByte(vl53l0_dev, 0x00C0, &ProductType);
	if (VL53L0_ERROR_NONE != rc)  {
		vl53l0_errmsg("vl53l0 doesn't exist\n");
		return ENODEV;
	} else {
		if (0xEE != ProductType) {
			vl53l0_errmsg("Product type is %d, this isn't vl53l0\n", ProductType);
			return ENODEV;
		}
	}
