...
unsigned int get_rf_gpio_value1(void)
{
        return last_eint_level1;
}
...
static irqreturn_t usbcable_irq_handler1(int irq, void *dev)
{
        int level = 0;
        //FUNCTION_ENTER;

        level = gpio_get_value(usbcable_gpio_num1);
        //PK_XLOG_INFO(" gpio_value=%d,last_value=%d\n",level,last_eint_level);
        if(level!=last_eint_level1){
                printk(KERN_ERR "usbcable: irq1 handler report key [%d]\n", level);
...
static int usbcable_probe(struct platform_device *pdev){
	...
	//get usbcable-gpio num and set gpio dir/debounce...
        usbcable_gpio_num1 = of_get_named_gpio(node, "usbcable-gpio1", 0);
        //ret = of_property_read_u32(node, "debounce", &usbcable_gpio_deb);
        //if (ret < 0) {
        //        PK_XLOG_ERR("gpiodebounce not found,ret:%d\n", ret);
        //        return ret;
    ...
    ret = gpio_request_one(usbcable_gpio_num1, GPIOF_IN,"usbcable-gpio1" "-int");
    if (ret) {
        PK_XLOG_ERR("Request INT gpio1 (%d) failed %d", usbcable_gpio_num1, ret);
        return ret;
    }

    ret = gpio_direction_input(usbcable_gpio_num1);
    if (ret < 0) {
        PK_XLOG_ERR("gpio1-%d input set fail!!!\n", usbcable_gpio_num1);
        return ret;
    }
	...
	ret = request_irq(usbcable_eint_num, usbcable_irq_handler,IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "USB_CABLE", NULL);
	if (ret) {
	...
	//usbcable_eint_num = irq_of_parse_and_map(node, 0);
        usbcable_eint_num1 = gpio_to_irq(usbcable_gpio_num1);
    if (usbcable_eint_num1 < 0) {
        PK_XLOG_ERR("Parse irq1 failed %d", ret);
        return usbcable_eint_num1;
    }
        PK_XLOG_INFO("usbcable_eint_num1<%d>\n",usbcable_eint_num1);
        ret = request_irq(usbcable_eint_num1, usbcable_irq_handler1,IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "USB_CABLE1", NULL);
        if (ret) {
                PK_XLOG_ERR("request_irq1 fail, ret:%d.\n",ret);
                return ret;
        }
        //enable_irq_wake(usbcable_eint_num);
        PK_XLOG_INFO("usbcable eint1 request success!\n");

        ret = gpio_get_value(usbcable_gpio_num1);
        if(ret!=last_eint_level1){
                printk(KERN_ERR "usbcable: probe report key1 [%d]\n", ret);
                input_report_key(cable_input_dev, KEY_TEEN, ret);
                input_sync(cable_input_dev);
                last_eint_level1 = ret;
        }
