...
/* #define LIS2DH_DBG */
#ifdef LIS2DH_DBG
#define lis2dh_logi(dev, fmt, ...)	dev_info(dev, fmt, ##__VA_ARGS__)
#define lis2dh_warn(dev, fmt, ...)	dev_warn(dev, fmt, ##__VA_ARGS__)
#define lis2dh_err(dev, fmt, ...)	dev_err(dev, fmt, ##__VA_ARGS__)
#else
#define lis2dh_logi(dev, fmt, ...)
#define lis2dh_warn(dev, fmt, ...)	dev_warn(dev, fmt, ##__VA_ARGS__)
#define lis2dh_err(dev, fmt, ...)	dev_err(dev, fmt, ##__VA_ARGS__)
#endif
...
static int lis2dh_acc_hw_init(struct lis2dh_acc_status *stat)
{
	...
	lis2dh_logi(stat->dev, "%s: hw init start\n", LIS2DH_ACC_DEV_NAME);
	...
	lis2dh_logi(stat->dev, "%s: hw init done\n", LIS2DH_ACC_DEV_NAME);

	return 0;

err_resume_state:
	stat->hw_initialized = 0;
	lis2dh_err(stat->dev, "hw init error 0x%02x,0x%02x: %d\n", buf[0],
		buf[1], err);
...
static void lis2dh_acc_device_power_off(struct lis2dh_acc_status *stat)
{
	...
	err = stat->tf->write(stat, CTRL_REG1, 1, &buf);
	if (err < 0)
		lis2dh_err(stat->dev, "soft power off failed: %d\n", err);
				      ...
				      u8 new_fs_range)
{
	...
	lis2dh_logi(stat->dev, "%s start!new_fs_range=0x%x\n",
	...
	case LIS2DH_ACC_G_16G:
		...
		break;
	default:
		lis2dh_err(stat->dev, "invalid fs range requested: %u\n",
			new_fs_range);
		return -EINVAL;
	}
				 ...
				 int poll_interval_ms)
{
	 ...
	 */
	lis2dh_logi(stat->dev, "%s start!poll_interval_ms=%d ms\n",
	...
	lis2dh_logi(stat->dev, "%s: stat->ktime=%lld ns\n",
				__func__, stat->ktime.tv64);
	return err;
}
...
static enum hrtimer_restart
			lis2dh_acc_poll_function_read(struct hrtimer *timer)
{
	...
	lis2dh_logi(stat->dev, "%s: stat->timestamp=%lld\n",
				__func__, stat->timestamp);
...
static void lis2dh_acc_report_values(struct lis2dh_acc_status *stat,
				     int *xyz)
{
	input_event(stat->input_dev, INPUT_EVENT_TYPE, INPUT_EVENT_X, -xyz[1]);
	input_event(stat->input_dev, INPUT_EVENT_TYPE, INPUT_EVENT_Y, xyz[0]);
	...
	lis2dh_logi(stat->dev, "ABS_X: %d, ABS_Y: %d, ABS_Z: %d\n",
				-xyz[1], xyz[0], xyz[2]);
}
				     ...
				     const char *buf, size_t size)
{
	...
	interval_ms = max_t(unsigned int,
			interval_ms, stat->pdata->min_interval);
	mutex_lock(&stat->lock);
	stat->pdata->poll_interval = interval_ms;
	lis2dh_acc_update_odr(stat, interval_ms);
	lis2dh_logi(stat->dev, "interval_ms = %lx\n", interval_ms);
	mutex_unlock(&stat->lock);
			      ...
			      const char *buf, size_t size)
{
	...
	if (kstrtoul(buf, 10, &val))
		return -EINVAL;
	lis2dh_logi(stat->dev, "%s start! val = %lx\n", __func__, val);
	switch (val) {
	case 2:
	...
	mutex_unlock(&stat->lock);
	lis2dh_logi(stat->dev, "range set to: %lu g\n", val);
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	lis2dh_logi(stat->dev, "%s start!, enabled =%x\n", __func__, val);
	return sprintf(buf, "%d\n", val);
}
			       ...
			       const char *buf, size_t size)
{
	...
	if (kstrtoul(buf, 10, &val))
		return -EINVAL;
	lis2dh_logi(stat->dev, "%s start! enabled val = %lx\n", __func__, val);
	if (val)
...
error:
	...
	lis2dh_err(dev, "%s:Unable to create interface\n", __func__);
...
static void lis2dh_acc_input_work_func(struct work_struct *work)
{
	...
	if (tmpkt <= 0) {
		/*
		 *set hrtimer_start expiry time to 0, start
		 *schedule work immediately to avoid delay.
		 */
		lis2dh_warn(stat->dev, "%s:negative timestamp tmpkt =%lld\n",
					__func__, tmpkt);
		hrtimer_start(&stat->hr_timer,
				ktime_set(0, 0), HRTIMER_MODE_REL);
	} else {
		lis2dh_logi(stat->dev, "%s:timestamp_debug tmpkt =%lld\n",
					__func__, tmpkt);
	...
	if (err < 0)
		lis2dh_err(stat->dev, "get_acceleration_data failed\n");
	else
...
static int lis2dh_acc_validate_pdata(struct lis2dh_acc_status *stat)
{
	...
	if (stat->pdata->axis_map_x > 2 || stat->pdata->axis_map_y > 2 ||
		...
		lis2dh_err(stat->dev, "invalid axis_map value x:%u y:%u z%u\n",
	...
	if (stat->pdata->negate_x > 1 || stat->pdata->negate_y > 1 ||
		...
		lis2dh_err(stat->dev, "invalid negate value x:%u y:%u z:%u\n",
	...
	if (stat->pdata->poll_interval < stat->pdata->min_interval) {
		lis2dh_err(stat->dev, "minimum poll interval violated\n");
		return -EINVAL;
	}
...
static int lis2dh_acc_input_init(struct lis2dh_acc_status *stat)
{
	...
	if (!stat->input_dev) {
		lis2dh_err(stat->dev, "input device allocation failed\n");
		return -ENOMEM;
	}
	...
	if (err) {
		lis2dh_err(stat->dev, "unable to register input device %s\n",
			stat->input_dev->name);
...
int lis2dh_acc_probe(struct lis2dh_acc_status *stat)
{
	...
	stat->hw_working = 1;
	stat->pdata =
		kmalloc(sizeof(struct lis2dh_acc_platform_data), GFP_KERNEL);
	if (!stat->pdata) {
		err = -ENOMEM;
		lis2dh_err(stat->dev,
	...
	if (stat->dev->platform_data == NULL) {
		memcpy(stat->pdata, &default_lis2dh_acc_pdata,
		       sizeof(*stat->pdata));
		lis2dh_logi(stat->dev, "using default plaform_data\n");
	} else {
	...
	if (err < 0) {
		lis2dh_err(stat->dev, "failed to validate platform data\n");
	...
	err = lis2dh_acc_device_power_on(stat);
	if (err < 0) {
		lis2dh_err(stat->dev, "power on failed: %d\n", err);
		goto exit_kfree_pdata;
	}
	...
	if (err < 0) {
		lis2dh_err(stat->dev, "update_odr failed\n");
		goto  err_power_off;
	}

	err = lis2dh_acc_input_init(stat);
	if (err < 0) {
		lis2dh_err(stat->dev, "input init failed\n");
		goto err_power_off;
	}
	...
	kobject_uevent(&stat->dev->kobj, KOBJ_ADD);
...
err_mutexunlock:
	mutex_unlock(&stat->lock);
	lis2dh_err(stat->dev, "%s: Driver Init failed\n", LIS2DH_ACC_DEV_NAME);
...
{
	lis2dh_logi(stat->dev, "driver removing\n");
...
int lis2dh_acc_common_resume(struct lis2dh_acc_status *stat)
{
	...
	lis2dh_logi(stat->dev, "%s start! stat->on_before_suspend = %d\n",
