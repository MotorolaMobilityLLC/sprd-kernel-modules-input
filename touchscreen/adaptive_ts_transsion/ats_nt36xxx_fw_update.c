...
static u8 fw_file0[] = {
#include TS36XXX_UPGRADE_FW0_FILE
};
static int32_t nvt_get_fw_need_write_size(const struct firmware *fw_entry)
{
	...
	for (i = total_sectors_to_check; i > 0; i--) {
		...
		if (strncmp(&fw_entry->data[i * FLASH_SECTOR_SIZE - NVT_FLASH_END_FLAG_LEN], "NVT", NVT_FLASH_END_FLAG_LEN) == 0) {
			...
			printk("[TS][NVT-ts] fw_need_write_size = %zu(0x%zx), NVT end flag\n", fw_need_write_size, fw_need_write_size);
			return 0;
		}
		...
		if (strncmp(&fw_entry->data[i * FLASH_SECTOR_SIZE - NVT_FLASH_END_FLAG_LEN], "MOD", NVT_FLASH_END_FLAG_LEN) == 0) {
			fw_need_write_size = i * FLASH_SECTOR_SIZE;
			printk("[TS][NVT-ts] fw_need_write_size = %zu(0x%zx), MOD end flag\n", fw_need_write_size, fw_need_write_size);
			return 0;
		}
	}
...
static int32_t nvt_bin_header_parser(const u8 *fwdata, size_t fwsize)
{
	...
	printk("[TS][NVT-ts] ovly_info = %d, ilm_dlm_num = %d, ovly_sec_num = %d, info_sec_num = %d, partition = %d\n",
...
static int32_t update_firmware_request(char *filename)
{
	uint8_t retry = 0;
	int32_t ret = 0;
	unsigned long ver_offset = FW_BIN_VER_OFFSET;
	unsigned long ver_bar_offset = FW_BIN_VER_BAR_OFFSET;
	if (NULL == filename) {
		return -ENOENT;
	}
	...
	while (1) {
		...
		if (*(fw_entry->data + ver_offset) + *(fw_entry->data + ver_bar_offset) != 0xFF) {
			NVT_ERR("bin file FW_VER + FW_VER_BAR should be 0xFF!\n");
			NVT_ERR("FW_VER=0x%02X, FW_VER_BAR=0x%02X\n", *(fw_entry->data+ver_offset), *(fw_entry->data+ver_bar_offset));
			ret = -ENOEXEC;
			goto invalid;
		}
...
static int32_t nvt_read_ram_and_save_file(uint32_t addr, uint16_t len, char *name)
{
	...
	nvt_spi_read(ts->client, fbufp, len+1);
		...
		uint32_t SRAM_addr, uint32_t size, uint32_t BIN_addr)
{
	...
	for (i = 0 ; i < count ; i++) {
		...
		ret = nvt_spi_write(ts->client, fwbuf, len+1);
		if (ret) {
...
static int32_t nvt_check_fw_checksum(void)
{
	...
	ret = nvt_spi_read(ts->client, fwbuf, len+1);
	if (ret) {
		...
		uint32_t G_CHECKSUM_ADDR, uint32_t crc)
{
	...
	nvt_spi_write(ts->client, fwbuf, 4);
	...
	if (ts->hw_crc == 1) {
		nvt_spi_write(ts->client, fwbuf, 3);
	} else if (ts->hw_crc > 1) {
		nvt_spi_write(ts->client, fwbuf, 4);
	}
	...
	fwbuf[3] = (crc >> 16) & 0xFF;
	fwbuf[4] = (crc >> 24) & 0xFF;
	nvt_spi_write(ts->client, fwbuf, 5);
...
#if 1
static int nvt_get_tpd_firmware(const struct firmware **firmware_p)
{
	int32_t ret = 0;
	struct firmware *fw_buf = NULL;
	//uint8_t nvt_cfgs_grp[16]={0};

	struct nvt_dts_info {
		int vendor_num;
		u32 *tpd_firmware_size;
	};
	struct nvt_dts_info ntv_dts_data= {
		.vendor_num=0,
		.tpd_firmware_size=0,
	};

	ntv_dts_data.tpd_firmware_size = kmalloc(ts->vendor_nums*sizeof(u32), GFP_KERNEL);
	if(ntv_dts_data.tpd_firmware_size == NULL){
		NVT_ERR("tpd_firmware_size kmalloc is not found\n");
		return -1;
	}

	ret = of_property_read_u32_array(ts->update_node, "tp_fw_buf_size", ntv_dts_data.tpd_firmware_size, ts->vendor_nums);
	if (ret)
		NVT_ERR("error reading tp_fw_buf_size. ret=%d\n", ret);

	fw_buf = kmalloc(sizeof(struct nvt_firmware), GFP_KERNEL);
	if(fw_buf == NULL){
	...
	/*fw_buf->size = ntv_dts_data.tpd_firmware_size[ts->vendor_num];
        //fw_buf->data = kmalloc(fw_buf->size, GFP_KERNEL);
	fw_data = kmalloc(fw_buf->size, GFP_KERNEL);
	NVT_LOG("fw_buf->size=%d\n", fw_buf->size);
	if(fw_data == NULL){
		NVT_ERR("ili_fw_entry->data malloc is not found\n");
		return -1;
	}

	sprintf(nvt_cfgs_grp, "tp_fw_buf%d", ts->vendor_num);
	NVT_LOG("nvt_cfgs_grp=%s\n",nvt_cfgs_grp);
	of_property_read_u8_array(ts->update_node, nvt_cfgs_grp, fw_data, ntv_dts_data.tpd_firmware_size[ts->vendor_num]);

	fw_buf->data = fw_data;*/
	fw_buf->size = sizeof(fw_file0);
...
int nvt_get_tpd_firmware_data(void)
{
	int32_t ret = 0;
	unsigned long ver_offset = FW_BIN_VER_OFFSET;
	unsigned long ver_bar_offset = FW_BIN_VER_BAR_OFFSET;
...
int32_t nvt_update_firmware(char *firmware_name)
{
	...
	if (g_fw_control){
		...
		if(times_once==1){
			ret = nvt_get_tpd_firmware(&fw_entry);
			if(ret!=0){
				NVT_ERR(" failed. (%d)\n", ret);
				goto request_firmware_fail;
			}
			ret = nvt_get_tpd_firmware_data();
			if(ret!=0){
				NVT_ERR(" failed123. (%d)\n", ret);
				goto request_firmware_fail;
			}
