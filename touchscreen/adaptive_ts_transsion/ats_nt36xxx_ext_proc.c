...
#define NVT_FW_CONTROL "nvt_fw_control"
...
static struct proc_dir_entry *NVT_proc_fw_control_entry;
/*******************************************************
Description:
...
void nvt_read_mdata(uint32_t xdata_addr, uint32_t xdata_btn_addr)
{
	...
	for (i = 0; i < ((dummy_len + data_len) / XDATA_SECTOR_SIZE); i++) {
		...
		for (j = 0; j < (XDATA_SECTOR_SIZE / BUS_TRANSFER_LENGTH); j++) {
			...
			nvt_spi_read(ts->client, buf, BUS_TRANSFER_LENGTH + 1);
	...
	nvt_spi_read(ts->client, buf, (TOUCH_KEY_NUM * 2 + 1));
...
static int32_t fw_control_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%d, ", g_fw_control);

	return 0;
}

const struct seq_operations nvt_fw_control_ops = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
	.show   = fw_control_show
};

/*******************************************************
Description:
	Novatek touchscreen /proc/nvt_fw_control open
	function.

return:
	n.a.
*******************************************************/
static int32_t nvt_fw_control_open(struct inode *inode, struct file *file)
{
	if (mutex_lock_interruptible(&ts->lock)) {
		return -ERESTARTSYS;
	}
	...
	return seq_open(file, &nvt_fw_control_ops);
}

static const struct proc_ops nvt_fw_control_fops = {
	.proc_open = nvt_fw_control_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = seq_release,
};
...
int32_t nvt_extra_proc_init(void)
{
	...
	NVT_proc_fw_control_entry = proc_create(NVT_FW_CONTROL, 0111, NULL,&nvt_fw_control_fops);
	if (NVT_proc_fw_control_entry == NULL) {
		NVT_ERR("create proc/%s Failed!\n", NVT_FW_CONTROL);
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/%s Succeeded!\n", NVT_FW_CONTROL);
