...
#define NT36528_LCD_NAME "lcd_nt36528_dj_120hz_mipi_hd"
...
const uint16_t gesture_key_array[] = {
	...
	KEY_POWER,  //GESTURE_SINGLE_CLICK
};
#endif
...
static ssize_t nvt_ts_firmware_version_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "chip id:nt36528,fw=0x%02X\n", ts->fw_ver);
}
...
static ssize_t nvt_gesture_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	NVT_LOG("supported_gesture_type=%d", ts->sys_gesture_type);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	switch (value) {
		case 0x20:
			...
			nvt_cmd_ext_store(0x7B, ts->sys_gesture_type);
			break;
		case 0x21:
	...
	NVT_LOG("sys_gesture_type=%d, ts_gesture=%d\n", ts->sys_gesture_type, ts->gesture);

	return err;
}
static DEVICE_ATTR(gesture, 0664, nvt_gesture_show, nvt_gesture_store);
#endif
...
static DEVICE_ATTR(ts_info, 0664, nvt_ts_firmware_version_show, NULL);
