...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	ret = of_property_read_u32(np, "spi-max-frequency", &ts->spi_max_frequency);
	if (ret < 0) {
		ts->spi_max_frequency = NVT_MAX_SPI_SPEED;
		NVT_ERR("Unable to get spi-max-frequency, set default value %d\n", ts->spi_max_frequency);
	}else {
		NVT_LOG("get spi-max-frequency, value = %d\n", ts->spi_max_frequency);
	}
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	ts->client->max_speed_hz = ts->spi_max_frequency;
	NVT_LOG("start to spi_setup, ts->client->bits_per_word=%d,ts->client->mode=%d,ts->client->max_speed_hz=%d\n"
			,ts->client->bits_per_word,ts->client->mode,ts->client->max_speed_hz);
