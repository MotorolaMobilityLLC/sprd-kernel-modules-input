...
#define LCD_NAME "lcd_nt36528_boe_mipi_hdp"
...
const uint16_t gesture_key_array[] = {
	/*bsp:add gesture key by jiangzhu.yang 20230711 start*/
	KEY_F13,
	KEY_F20,
	/*bsp:add gesture key by jiangzhu.yang 20230711 end*/
...
void nvt_ts_suspend_cali_autotest(struct device *dev);
/*******************************************************
Description:
...
info_retry:
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		if (retry_count < 3) {
			...
			ts->fw_ver = 0;
			//BSP: modify for tp resolution by hongwu.zhan 20230423 start
			ts->abs_x_max = ts->tp_resolution[0];
			ts->abs_y_max = ts->tp_resolution[1];
			//BSP: modify for tp resolution by hongwu.zhan 20230423 end
...
#if WAKEUP_GESTURE
/*bsp:modify for bringup by guoxin.zhu 20230626 start*/
/* function page definition */
#define FUNCPAGE_GESTURE         1
/*bsp:modify for bringup by guoxin.zhu 20230626 end*/
...
void nvt_ts_wakeup_gesture_report(uint8_t gesture_id, uint8_t *data)
{
    ...
    // int ret = 0;
    // /*BSP: add transsion core by guoxin.zhu 20230613 start*/
	// struct tran_core_ops *ops = NULL;
    // /*BSP: add transsion core by guoxin.zhu 20220613 end*/
...
/*BSP: add transsion core by guoxin.zhu 20230613 start*/
    // if(get_tpd_ops(&nt36xxx_controller, &ops)) {
    //     ret = ops->gesture_id_report(gesture_id);
    //     if (ret < 0) {
    //         NVT_ERR("[GESTURE]gesture touch data report fail");
    //         return;
    //     }
    // }
...
#endif
/*BSP: add transsion core by guoxin.zhu 20230613 end*/
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	//BSP: modify for tp resolution by hongwu.zhan 20230423 start
	ret = of_property_read_u32_array(np, "novatek,resolution",ts->tp_resolution,ARRAY_SIZE(ts->tp_resolution));
	if (ret) {
		NVT_ERR("error reading tp resolution. ret=%d\n", ret);
	} else {
		NVT_LOG("resulution is %d %d",ts->tp_resolution[0],ts->tp_resolution[1]);
	}
	//BSP: modify for tp resolution by hongwu.zhan 20230423 end
...
// /*bsp: add for tp proximity function by guoxin.zhu 20230712 start*/
// static int report_ps_state(uint8_t* point_data)
// {
// 	struct tran_core_ops *ops = NULL;
// 	u32 last_state;
// 	u32 psensor_status = 0;

// 	/* get core ops && check ps switch */
// 	if(!get_tpd_ops(&nt36xxx_controller, &ops) || ops->ps_state_get(NULL) != TRAN_PS_ON) {
// 		return 0;
// 	}

// 	/* get last ps state ,near or far */
// 	ops->ps_state_get(&last_state);
// 	/* get current ps state */

// 	if (((point_data[1]==0xF0) && (point_data[2]==0x02))&&((point_data[3] != 0x01) && (point_data[3] != 0x02))){
// 		NVT_LOG("reset ps mode point_data[3]=0x%x\n", point_data[3]);
// 	}

// 	if((point_data[1]==0xF0) && (point_data[2]==0x02)){

//         if (0x01 == point_data[3]){//near
//         	psensor_status = TRAN_PS_NEAR;
//         }
//         else if (0x02 == point_data[3]){//far-away
//             psensor_status = TRAN_PS_FAR;
//         }
//         else{
//         	NVT_LOG("point_data[3] data error /ps failed!!\n");
//         }

//         if(last_state != psensor_status) {
//             NVT_LOG("ps %s report",(psensor_status == TRAN_PS_NEAR)?ENUM_TO_STRING(TRAN_PS_NEAR):ENUM_TO_STRING(TRAN_PS_FAR));
//         	ops->ps_state_report(psensor_status);

//         } else {
//         	//NVT_LOG("ps report ignore cur:0x%x last:0x%x ",psensor_status, last_state);
//         	return 0;
//         }
//     }
...
// 	return 0;
// }
// /*bsp: add for tp proximity function by guoxin.zhu 20230712 end*/
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	int32_t retry = 0;
#endif
/*bsp:add for transsion core by guoxin.zhu 20230626 start*/
    // struct tran_core_ops *ops = NULL;
/*bsp:add for transsion core by guoxin.zhu 20230626 end*/
    ...
    // /*BSP: modify fo bringup by guoxin.zhu 20230626 start*/
    // nt36xxx_controller.input_dev = ts->input_dev;
    // /*BSP: modify fo bringup by guoxin.zhu 20230626 start*/
    ...
    /*BSP: modify fo bringup by guoxin.zhu 20230626 end*/
#endif
    ...
    // /*BSP: modify fo bringup by guoxin.zhu 20230626 start*/
    // if(get_tpd_ops(&nt36xxx_controller, &ops)) {
    //     ops->controller_init_completion();
    // }
    // /*BSP:modify fo bringup by guoxin.zhu 20230626 end*/
...
#if WAKEUP_GESTURE
    // if(nt36xxx_controller.tran_ts_data->ts_core_mode == TRAN_GESTURE_SUSPEND_MODE){
    //     //---write command to enter "wakeup gesture mode"---
    //     buf[0] = EVENT_MAP_HOST_CMD;
    //     buf[1] = 0x13;
    //     CTP_SPI_WRITE(ts->client, buf, 2);
    //     enable_irq_wake(ts->client->irq);
    //     NVT_LOG("touch suspend wakeup gesture mode\n");
    // } else {
        //---write command to enter "deep sleep mode"---
        buf[0] = EVENT_MAP_HOST_CMD;
        buf[1] = 0x11;
        CTP_SPI_WRITE(ts->client, buf, 2);
        NVT_LOG("touch suspend deep sleep mode\n");
    // }
#else // WAKEUP_GESTURE
	//---write command to enter "deep sleep mode"---
	buf[0] = EVENT_MAP_HOST_CMD;
...
static struct of_device_id nvt_match_table[] = {
    ...
    /*bsp:add for sprd bringup by guoxin.zhu 20230626 start*/
	{.compatible = "adaptive-touchscreen",},
	/*bsp:add for sprd bringup by guoxin.zhu 20230626 end*/
	{ },
};
#endif
...
/*bsp:modify for sprd bringup by guoxin.zhu 20230626 start*/
...
module_exit(nvt_driver_exit);
#endif
/*bsp:modify for sprd bringup by guoxin.zhu 20230626 end*/
