	...
	unsigned short reg, unsigned char *data, unsigned short length)
{
	...
	while(iRtyCount){
		if(!tp_i2c_safaMode)
			break;
		mdelay(200);
		TS_WARN("F:%s L:%d irqEnable:%d iRtyCount:%d\n",
...
int ts_i2c_init(struct device_node *pn, unsigned short *addrs)
{
	...
	TS_DEBUG("init i2c in %sadaptive mode", addrs ? "" : "non-");
...
EXPORT_SYMBOL(ts_i2c_init);
EXPORT_SYMBOL(ts_i2c_exit);
