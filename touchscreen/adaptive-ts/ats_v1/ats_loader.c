...
#define TS_PROP_PANEL_PARENT		"touchpanel-parent"
#define TS_PROP_SURFACE_PANELWIDTH	"sprd,surface-width"
#define TS_PROP_SURFACE_PANELHEIGHT	"sprd,surface-height"
...
static struct ts_board *ts_parse_dt(struct device_node *pn)
{
	...
	retval = of_property_read_u32(pn, TS_PROP_PANEL_PARENT, &touchpanel_parent);
	if (!retval) {
		np = of_find_node_by_phandle(touchpanel_parent);
		if (np) {
			retval = of_property_read_u32(np,
						TS_PROP_SURFACE_PANELWIDTH, &board->surface_width);
			retval |= of_property_read_u32(np,
