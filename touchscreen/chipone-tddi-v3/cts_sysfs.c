        ...
        struct device_attribute *attr, char *buf)
{
    ...
    return snprintf(buf, PAGE_SIZE, "chip id:icnl9922c,fw=%04x\n",
        cts_data->cts_dev.fwdata.version);
}
static DEVICE_ATTR(ts_info, S_IRUGO, ts_info_show, NULL);
...
void cts_gesture_state_switch(void)
{
	if (g_cts_data->sys_gesture_type) {
		//gesture enable
		if (!g_cts_data->cts_dev.rtdata.gesture_wakeup_enabled) {
			g_cts_data->cts_dev.rtdata.gesture_wakeup_enabled = true;
			touch_set_state(TOUCH_LOW_POWER_STATE);
			cts_info("gesture switch to enable");
		}
	}
	else {
		//gesture disable
		if (g_cts_data->cts_dev.rtdata.gesture_wakeup_enabled) {
			g_cts_data->cts_dev.rtdata.gesture_wakeup_enabled = false;
			touch_set_state(TOUCH_DEEP_SLEEP_STATE);
			cts_info("gesture switch to disable");
		}
	}
}
                                             ...
                                             const char *buf, size_t count)
{
        ...
        switch (value) {
                case 0x20:
                        ...
                        cts_data->sys_gesture_type &= 0xFE;
                        cts_data->s_tap_flag = 0;
                        break;
                case 0x21:
                        cts_info("[%s %d]:  single tap enable", __func__, __LINE__);
                        cts_data->sys_gesture_type |= 0x01;
                        cts_data->s_tap_flag = 1;
                        break;
                case 0x30:
                        cts_info("[%s %d]:  double tap disable", __func__, __LINE__);
                        cts_data->sys_gesture_type &= 0xFD;
                        cts_data->d_tap_flag = 0;
                        break;
                case 0x31:
                        cts_info("[%s %d]:  double tap enable", __func__, __LINE__);
                        cts_data->sys_gesture_type |= 0x02;
                        cts_data->d_tap_flag = 1;
                        break;
                default:
                        cts_info("[%s %d]: unsupport gesture mode type", __func__, __LINE__);
        }
        cts_gesture_state_switch();
        cts_info("sys_gesture_type=%d, gesture=%d\n", cts_data->sys_gesture_type, cts_data->cts_dev.rtdata.gesture_wakeup_enabled);
        cts_unlock_device(&cts_data->cts_dev);

        return count;
}
static DEVICE_ATTR(gesture, S_IRUSR | S_IWUSR, gesture_show,
        gesture_store);
#endif
