...
#define ICNL9922C_LCD_NAME "lcd_icnl9922c_boe_120hz_mipi_fhd"
...
#ifdef CONFIG_CTS_I2C_HOST
static void cts_driver_shutdown(struct i2c_client *client)
#else
...
#endif
{
    ...
    if (gpio_is_valid(cts_data->pdata->rst_gpio)) {
        gpio_set_value(cts_data->pdata->rst_gpio, 0);
        cts_info("cts_driver_shutdown set rst gpio low");
    }
...
static int get_bootargs(char *current_mode, char *boot_param)
{
	...
	if (!np) {
		cts_err("Can't get the /chosen");
		return -EIO;
	}

	ret = of_property_read_string(np, "bootargs", &cmd_line);
	if (ret < 0) {
		cts_err("Can't get the bootargs");
		return ret;
	}
	...
	if(s){
		...
		*current_mode = '\0';
		...
		cts_err("Read bootargs %s fail",boot_param);
		return 1;
	}
	return 0;
}
...
static int __init cts_driver_init(void)
{
    ...
    if (!ret && !(strstr(lcd_name, ICNL9922C_LCD_NAME)))
    {
        cts_err("%s: match %s fail,return",__func__,lcd_name);
        return -ENODEV;
    }
