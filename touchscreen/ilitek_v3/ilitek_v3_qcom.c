...
#define LCD_NAME "lcd_ili9883c_txd_120hz_mipi_hd"
...
int ili_irq_register(int type)
{
	...
	ret = devm_request_threaded_irq(ilits->dev, ilits->irq_num,
				   ...
				   type | IRQF_ONESHOT | IRQF_NO_SUSPEND, "ilitek", NULL);
...
static int get_bootargs(char *current_mode, char *boot_param)
{
	...
	if (!np) {
		ILI_ERR("Can't get the /chosen\n");
		return -EIO;
	}

	ret = of_property_read_string(np, "bootargs", &cmd_line);
	if (ret < 0) {
		ILI_ERR("Can't get the bootargs\n");
		return ret;
	}
